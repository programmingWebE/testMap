...
$('.catalog__input').on('input', function() {
  $(this).val($(this).val().replace(/[A-Za-zА-Яа-яЁё]/, ''));
});

$('.catalog__input').on('blur', function() {
...
card_gallery.on('click', function(swiper, e) {
  ...
  $('.catalog__btn-mob').on('click', function(e){
   $(e.target).parents('.popup').removeClass('popup--active');
   $('.wrap').removeClass('wrap--active');
  })

  $('.popup').on('click', '.catalog__btn-mob', function(e) {
   $('.popup').removeClass('popup--active');
    $('.wrap').removeClass('popup--bg');
  })
  ...
  $('.card__btn--buy').on('click', function(){
    ...
    if ($('.popup--order').is(e.target)  && $('.popup--size').has(e.target).length === 0 ){
   $('.popup').removeClass('popup--active');
    $('.wrap').removeClass('wrap--active');
  }
  })

  $('.popup__close-order').on('click', function(){
    $('.popup').removeClass('popup--active');
    $('.wrap').removeClass('wrap--active');
  })
...
$('.popup__inner').on('click', '.popup__title--active', function(){
 ...
 ReadMoreText('.reviews__item', '.reviews__more', '.reviews__text', '.reviews__block', 'reviews__text--active reviews__text--gradient', 'reviews__text--active', '114px');
  ...
  if($('#map').length>0) {
    ...
    function loadScript() {
      const script_google_map = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyC8I82DhSjgntcm5mDcEvM1iIrpCm4NGwg';
     ...
     // if ($(window).scrollTop() >= (iframeContainerOffsetTop - windowHeight - advancedOffset)) {
       ...
       // loadScript(script_google_map, map_init);
      //}
 ...
 let map;
  ...
  $(document).on('input', '.select__input', function(e) {
    let isFound;
    $(e.target).parents('.select__list').find('.select__item').each((i, el) => {
      let is = $(el).html().toLowerCase().indexOf(e.target.value.toLowerCase()) != -1;
